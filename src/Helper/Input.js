...
const Input = ({
  ...
  if (!!disabled) {
    return (
      <div className="w-full my-3  relative flex flex-col items-start justify-center">
        ...
        >{title}</label>
        <input
          className=" bg-gray-200 border border-gray-400 text-gray-900  rounded-lg  block w-full p-2.5"
          {...rest}
        />
      </div>
    )
  ...
  return (
    <div className="w-full my-3  relative flex flex-col items-start justify-center">
      ...
      <input
        className={` ${preInput && "text-right"} bg-gray-50 border border-gray-300 text-gray-900  rounded-lg focus:ring-blue-500 text-base focus:border-blue-500 block w-full p-2.5 `}
        type={type}
